...
const createOTP = (req, res) => {
    client.verify.v2
        ...
        .verifications
        .create({ to: formatPhonenum(req.query.phonenum), channel: "sms" })
...
const verifyOTP = (req, res) => {
    client.verify.v2
        .services(otp.verifySid)
        .verificationChecks
        .create({ to: formatPhonenum(req.query.phonenum), code: req.query.code })
...
const getExistPhonenumber = (req, res) => {
    pool.getConnection((err, db) => {
        ...
        db.query("SELECT IF((SELECT phone_number, role_id FROM user_t WHERE phone_number = ? LIMIT 1), 'exist', 'notexist') AS isExist", [phonenum],
        (err, result) => {
            if (err) {
                console.log(err);
            } else {
