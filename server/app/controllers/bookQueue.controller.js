...
const BookQueue = (req, res) => {
  pool.getConnection((err, db) => {
    ...
    db.query(
      `INSERT INTO queue_log (restaurant_name, phone_number, menu_name, ingredient, note, order_status, queue_id) 
        VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [
        restaurant_name,
        phone_number,
...
const getQueue = (req, res) => {
  pool.getConnection((err, db) => {
    ...
    db.query(
      `SELECT q.queue_id, (SELECT COUNT(*) FROM queue_log WHERE order_status = 0 AND restaurant_name = q.restaurant_name 
